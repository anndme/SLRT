...
class LogisticSecretShare(object):
    ...
    def generating_matrix(self, x, y, n):
        ...
        B = np.hstack((np.ones((1, n - half_n - 2)), B.reshape(1, B.shape[0])))
        B_1 = np.insert(B, len(B), y, axis=1)
        B_2 = np.insert(B, 0, [1], axis=1)
        Ma = np.dot(A_1.reshape(A_1.shape[1], A_1.shape[0]), A_2)
        Mb = np.dot(B_1.reshape(B_1.shape[1], B_1.shape[0]), B_2)
    ...
    def share_matrix(self, x, y, n):
        A0, A1, B0, B1 = self.generating_matrix(x, y, n)
        U = np.random.randint(low=0, high=10, size=(self.n, self.n))
        V = np.random.randint(low=0, high=10, size=(self.n, self.n))
        Z = np.dot(U, V)   # 第一步（1）
        # 第二步
        ...
        # 第五步
        # SO
        C0 = np.dot(0*E, F) + np.dot(U0, F) + np.dot(E, V0) + Z0
        C1 = np.dot(1*E, F) + np.dot(U1, F) + np.dot(E, V1) + Z1
        return C0, C1
    ...
    def omega_zero(self, omega_init_r, zr, u0):
        omega_new = (1 - 2 * self.lanta * self.rate) * omega_init_r - self.rate / self.size * (0.17132 * zr - 0.5 * u0)
        return omega_new
...
if __name__ == '__main__':
    ...
    for j in [3, 10]:
        ...
        for i in range(100):
            ...
            A0, A1 = A.share_matrix(X_train, Y_train, X_train.shape[1] + 1)
            end = time.time()
